...
def plot_anomalies(ref, pred, anomalies, anomalies_pred, show_idx=0, threshold=0.5, ref_dims=(28, 28, 1)):
    ...
    axis[0][2].imshow(np.abs(pred[show_idx] - ref[show_idx]).reshape((square_shape, square_shape)))
    ...
    axis[1][2].imshow(np.abs(anomalies_pred[show_idx] - anomalies[show_idx]).reshape((square_shape, square_shape)))
